...
d_cl=8192
p_cls="256 512 1024 2048 4096 8192"
...
for p_cl in $p_cls; do
    for d_bs in $d_batch_sizes; do
        ...
        ncu --metrics $NCU_METRICS -k "regex:.*fwd.*" --csv $TEST_CMD --stage fused --p_bs 1 --p_cl $p_cl --p_cs $p_cs --d_bs $d_bs --d_cl $d_cl > output/fused-p-$p_cl-d-$d_bs-$d_cl.csv
        ncu --metrics $NCU_METRICS -k "regex:.*fwd.*" --csv $TEST_CMD --stage prefill --p_bs 1 --p_cl $p_cl --p_cs $p_cs --d_bs $d_bs --d_cl $d_cl > output/prefill-p-$p_cl-d-$d_bs-$d_cl.csv
        ncu --metrics $NCU_METRICS -k "regex:.*fwd.*" --csv $TEST_CMD --stage decode --p_bs 1 --p_cl $p_cl --p_cs $p_cs --d_bs $d_bs --d_cl $d_cl > output/decode-p-$p_cl-d-$d_bs-$d_cl.csv
    done
done